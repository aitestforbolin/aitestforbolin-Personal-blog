(function () {
  ...
  function trimSentence(text, maxLength) {
    ...
    return (boundary >= maxLength * 0.55 ? clipped.slice(0, boundary) : clipped)
      .replace(/[，；。\s]+$/, "") + "。";
  }
  ...
  function copyFinalText(text) {
    ...
    if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
      ...
      nativeWrite.catch(() => {
        if (!fallbackSucceeded) setLongPostStatus("X 长帖复制失败", "error");
      });
  ...
  function setLongPostStatus(message, level) {
    const status = document.querySelector("[data-copy-status]");
    if (!status) return;
    status.textContent = message || "已复制 1 篇 X 长帖";
    status.dataset.level = level || "success";
  }

  function suppressOriginalFallbackCopy() {
    const originalExecCommand = document.execCommand;
    let restored = false;
    try {
      document.execCommand = function (command) {
        if (String(command).toLowerCase() === "copy") return true;
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (!button || bypassCapture || button.disabled) return;

      event.preventDefault();
      event.stopImmediatePropagation();

      button.copyPayload = "";
      const restoreExecCommand = suppressOriginalFallbackCopy();
      bypassCapture = true;
      try {
        button.click();
      } finally {
      ...
      const threadText = String(button.copyPayload || "");
      if (!threadText) {
        setLongPostStatus("X 长帖生成失败", "error");
        return;
      }
      ...
      try {
        copyFinalText(longPost);
        setLongPostStatus("已复制 1 篇 X 长帖", "success");
        window.setTimeout(() => {
          try {
            if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
            ...
            // The synchronous copy above already handled the user gesture path.
          }
          setLongPostStatus("已复制 1 篇 X 长帖", "success");
        }, 120);
      } catch (error) {
        setLongPostStatus("X 长帖复制失败", "error");
      }
    },
    true
  );
