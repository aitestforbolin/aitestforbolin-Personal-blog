...
class XPublisherTests(unittest.TestCase):
    ...
    def test_workflow_only_automates_completed_snapshot_updates(self):
        ...
        self.assertIn("PUBLISH_MODE: ${{ steps.gate.outputs.publish_mode }}", workflow)
        self.assertIn("INPUT_AS_OF: ${{ steps.gate.outputs.input_as_of }}", workflow)
        self.assertIn("git diff-tree --root", workflow)
        self.assertIn("grep -Fx 'data/daily-market-status.json'", workflow)
        self.assertIn("grep -Fx 'data/x-publish-request.json'", workflow)
        self.assertNotIn("rg -x", workflow)
